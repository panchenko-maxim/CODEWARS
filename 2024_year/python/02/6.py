...
the error rate of the printer as a string representing a rational whose numerator is the number of errors and
...
def printer_error(s):
    # alphabet_true = "abcdefghijklm"
    alphabet_false = "nopqrstuvwxyz"
    count_wrong_alpha = 0
    for letter in alphabet_false:
        for el in s:
            if letter == el:
                count_wrong_alpha += 1
    return f"{count_wrong_alpha}/{len(s)}"
