...
To be a senior, a member must be at least 55 years old and have a handicap
 greater than 7. In this croquet club, handicaps range from -2 to +26; the better the player the lower the handicap.

Input
Input will consist of a list of pairs. Each pair contains information for
 a single potential member. Information consists of an integer for the person's
...
function openOrSenior(data){
    let lst = []
    for (let el of data){
        if (el[0] >= 55 && el[1] > 7) lst.push("Senior");
        else lst.push("Open");
    }
    return lst;
  }
